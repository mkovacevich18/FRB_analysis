    ...
    for dt in ${time_window[@]}; do
        for c in ${choose[@]}; do
	       ...
	       for s in ${COUNTER[@]}; do 
		    ...
		    echo "python3 /data/user/mkovacevich/FRB_analysis/FRB_121102_stacking_sens.py --gamma ${gamma} --dt ${dt} --seed ${s} --choose ${c} --bg_trials 2000.0 --sig_trials 100.0 --cpus 2.0" >> ${exec_path}
