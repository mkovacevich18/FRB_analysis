...
dag_path="$base/job_files/dags/FRB_Catalog_1_trials.dag"
touch ${dag_path}
    ...
    for dt in ${time_window[@]}; do
        for c in ${choose[@]}; do
	       ...
	       for s in ${COUNTER[@]}; do 
		    ...
		    echo "python3 $python_script_path/FRB_stacking_sensitivity.py --gamma ${gamma} --dt ${dt} --seed ${s} --choose ${c} --bg_trials 10000.0 --sig_trials 100.0 --cpus 1.0" >> ${exec_path}
